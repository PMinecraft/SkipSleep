...
/**
 * Update 子命令处理器
 * 处理 /skipsleep update <on|off>
 */
public class UpdateHandler implements SubCommandHandler {

    private final SkipSleep plugin;
    private final MessageService messageService;

    public UpdateHandler(SkipSleep plugin, MessageService messageService) {
        this.plugin = plugin;
    ...
    public String getPermission() {
        return "skipsleep.command.update";
    }
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        switch (args[0].toLowerCase()) {
            case "on":
            case "true":
                plugin.getConfigManager().setUpdateCheckEnabled(true);
                break;
            case "off":
            case "false":
                plugin.getConfigManager().setUpdateCheckEnabled(false);
                break;
            default:
